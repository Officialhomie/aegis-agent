...
| **Bearer token** | `Authorization: Bearer <AEGIS_API_KEY>` — agent, cycle, protocol agents, deposit-verify, reactive event, sponsorship request |
...
Protocol stats: budget, agents, 24h and 7d activity.
...
```json
{
  ...
  "name": "My Protocol",
  "tier": "gold",
  "budget": { "balanceUSD": 500, "totalSpent": 120, "sponsorshipCount": 42 },
  "agents": { "total": 5, "active": 3 },
  "activity": {
    "last24h": { "sponsorships": 8, "totalCostUSD": 0.12, "uniqueUsers": 6 },
    "last7d": { "sponsorships": 35, "totalCostUSD": 0.48 }
  },
...
Dry-run eligibility check (no side effects).
...
```json
{
  ...
  "errors": [],
  "warnings": [],
  "appliedRules": ["user-legitimacy-check", "approved-agent-check", ...],
  "checkedAt": "..."
}
```
...
Queue a sponsorship request for async processing.
