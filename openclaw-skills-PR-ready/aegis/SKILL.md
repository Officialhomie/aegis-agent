...
## OpenClaw Direct Commands

Aegis supports real-time conversation via OpenClaw's messaging bridge (`POST /api/openclaw`).

### Setup

```bash
export AEGIS_URL="https://your-aegis-instance.vercel.app"
export AEGIS_API_KEY="your-api-key"
```
...
```bash
curl -s -X POST "$AEGIS_URL/api/openclaw" \
  -H "Authorization: Bearer $AEGIS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"command": "status", "sessionId": "my-session-1"}'
```

### With callback URL (for async responses)

```bash
curl -s -X POST "$AEGIS_URL/api/openclaw" \
  -H "Authorization: Bearer $AEGIS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"command": "cycle", "sessionId": "my-session-1", "callbackUrl": "https://your-openclaw-callback.ngrok.io"}'
```

### Capability manifest

```bash
curl -s "$AEGIS_URL/api/openclaw" | jq .
```
...
| `sponsor the next N txs on base for uniswap v4` | Start targeted campaign (sponsor next N transactions) |
| `campaign status` | Show active campaign progress and report |
| `sponsor 0x<wallet> <protocol>` | Manually queue a sponsorship |
...
If you include a `callbackUrl` in your request, Aegis will POST autonomous action summaries to it:

```json
{ "agent": "aegis", "message": "[gas-sponsorship] SPONSOR_TRANSACTION: ...", "timestamp": "..." }
```

Rate limited to 1 notification per 5 minutes per session.
